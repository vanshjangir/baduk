...
func checkEmptyChains(blk chan int, wht chan int, empty chan Piece, wg *sync.WaitGroup) {
	...
	for lib := range empty {
		...
		chain := make(map[Piece]bool, 1)
		emptyCrawler(lib, chain)
